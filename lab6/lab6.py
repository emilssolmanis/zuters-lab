...
def split_covered(cpx, samples):
    try:
        covered = array(reduce(intersect1d, (where(samples[:, attr] == val) for attr, val in cpx.rules)))[0]
    except TypeError:
...
def statistical_significance(cpx, samples, classes):
    ...
    overall_hist = array([size(where(samples[:, 0] == c)) for c in classes])
    overall_hist *= float(len(covered)) / len(samples)
    return 2 * sum(f * log(f / g) for f, g in zip(covered_hist, overall_hist) if f != 0 and g != 0)
...
def find_best_cpx(samples, max_star, classes, stat_sig_thr):
    star = [Complex()]
    sels = [Complex(attr_idx, int(val)) for attr_idx, col in enumerate(samples.transpose()) for val in unique(col)]
        ...
        for cpx in star2:
            if statistical_significance(cpx, samples, classes) > stat_sig_thr:
                if laplace(cpx, samples, classes) > laplace(best_cpx, samples, classes):
                    best_cpx = cpx
        star = sorted(star2, key=lambda cpx: laplace(cpx, samples, classes))[:max_star]

    return best_cpx


def cn2(samples, max_star, stat_sig_thr=1.0):
    classes = unique(samples[:, 0])
    rule_list = []
    c = __most_common_class(samples)
    default_rule = 'ELSE <%d>' % c

    while True:
        best_cpx = find_best_cpx(samples, max_star, classes, 1.0)
        if len(best_cpx) > 0:
            covered_samples, uncovered_samples = split_covered(best_cpx, samples)
            c = __most_common_class(covered_samples)
            rule = 'IF <%s> THEN <%s>' % (best_cpx, c)
